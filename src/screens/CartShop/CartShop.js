...
export default class CartShop extends ValidationComponent {
    ...
    constructor(props) {
      super(props);         
      this.state = { payments:[], carShop:[], total:0, currentCard: '', fullMap: false, onchange: false, loading: true, loadingData: true, expMonth: '', expYear:'', csv: '', region: { latitude: LATITUDE, longitude: LONGITUDE, latitudeDelta: LATITUDE_DELTA, longitudeDelta: LONGITUDE_DELTA }, title:'' }       
    }        
    ...
    getData(){
      ...
      .then(async (response) => {  
        ...
        if (!cart.length){
          ...
          this.setState({carShop: cart, total: subtotal, loadingData: false });
        }              
      })
      .catch((error) => { 
    ...
    onRegionChange(region,title) {     
      ...
      // Alert.alert("QuickMap",JSON.stringify(region))
    }

    onRegionChangeComplete(region,title){
      this.setState({ region })         
      this.setState({ fullMap: false })
      // Alert.alert("FullMap",title)
    }
    ...
    render(){
      const { onchange, loading, total, carShop, payments, fullMap, region, loadingData } = this.state;                                 
        if (onchange) {
          return (
        ...
        return(          
            ...
            <ImageBackground source={require('src/assets/images/background.png')} style={styles.background}>
              ...
              { fullMap ? <Maps title={this.state.title} region={this.state.region} confirm={this.onRegionChangeComplete.bind(this)}/> : <Content refreshControl={
                  <RefreshControl
                ...
                <View style={{flexDirection:'row', borderBottomColor:'white', borderTopColor:'transparent', borderLeftColor:'transparent', borderRightColor:'transparent', borderWidth:0.8, width:'100%'}}>
                  ...
                  <Right style={{padding:10}}>
                    <Text style={[styles.text, {fontWeight:'bold'}]}>${(total + 25).toFixed(2)}</Text>                            
                  </Right>
                </View>                             
                { this.renderPayment() }
                { !loading ? <Mapa full={this.openMap.bind(this)} region={this.state.region} confirm={this.onRegionChange.bind(this)}/> : <LoadingScreen/> } 
                { payments.length ? 
